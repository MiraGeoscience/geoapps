...
class TensorMeshIO:
    ...
    def writeVTK(mesh, fileName, models=None):
        ...
        # Import
        from vtk import (
            vtkRectilinearGrid as rectGrid,
            vtkXMLRectilinearGridWriter as rectWriter,
            VTK_VERSION,
        )
        ...
        if ext == "":
            ...
            raise OSError("{:s} is an incorrect extension, has to be .vtr")
        # Write the file.
    ...
    def writeUBC(mesh, fileName, models=None):
        ...
        origin = mesh.x0 + np.array(
            [0, 0, mesh.hz.sum()]
        )  # Have to it in the same operation or use mesh.x0.copy(), otherwise the mesh.x0 is updated.
        origin.dtype = float
...
class TreeMeshIO:
    def writeUBC(mesh, fileName, models=None):
        ...
        # Shift index up by 1
        ubcCellPt = cellPointers[:, 0:-1].copy() + np.array([1.0, 1.0, 1.0])
        ...
        ubcReorder = np.argsort(
            ubcCellPt.view(",".join(3 * ["float"])), axis=0, order=["f2", "f1", "f0"]
        )[:, 0]
        # Make a array with the pointers and the withs, that are order in the ubc ordering
        indArr = np.concatenate(
            (ubcCellPt[ubcReorder, :], cellW[ubcReorder].reshape((-1, 1))), axis=1
        )
    ...
    def readUBC(TreeMesh, meshFile):
        ...
        # Extract the data
        nCunderMesh = np.array(fileLines[0].split("!")[0].split(), dtype=float)
        ...
        if np.unique(nCunderMesh).size > 1:
            raise Exception(
                "SimPEG TreeMeshes have the same number of cell in all directions"
            )
        tswCorn = np.array(fileLines[1].split("!")[0].split(), dtype=float)
        smallCell = np.array(fileLines[2].split("!")[0].split(), dtype=float)
        nrCells = np.array(fileLines[3].split("!")[0].split(), dtype=float)
        ...
        simpegCellPt = simpegCellPt - np.array([1.0, 1.0, 1.0])

        # Calculate the cell level
        simpegLevel = np.log2(np.min(nCunderMesh)) - np.log2(indArr[:, 3])
        # Make a pointer matrix
        simpegPointers = np.concatenate(
            (simpegCellPt, simpegLevel.reshape((-1, 1))), axis=1
        )

        ## Make the tree mesh
        mesh = TreeMesh([h1, h2, h3], x0)
        mesh._cells = {mesh._index(p) for p in simpegPointers.tolist()}
    ...
    def readModelUBC(mesh, fileName):
        ...
        assert hasattr(
            mesh, "_simpegReorderUBC"
        ), "The file must have been loaded from a UBC format."
        assert mesh.dim == 3
    ...
    def writeVTK(mesh, fileName, models=None):
        ...
        if str(type(mesh)).split()[-1][1:-2] not in "SimPEG.Mesh.TreeMesh.TreeMesh":
            raise OSError("mesh is not a SimPEG TreeMesh.")
        ...
        cellsArr.SetCells(
            cellConn.shape[0], numpy_to_vtkIdTypeArray(cellsMat, deep=True)
        )
        ...
        cellSides = np.array(
            [
                np.array(vtuObj.GetCell(i).GetBounds())
                .reshape((3, 2))
                .dot(np.array([-1, 1]))
                for i in np.arange(vtuObj.GetNumberOfCells())
            ]
        )
        uniqueLevel, indLevel = np.unique(
            np.prod(cellSides, axis=1), return_inverse=True
        )
