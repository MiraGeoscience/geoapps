...
def test_truncate_locs_depths():
    ...
    depth_core = DataInterpolationDriver.minimum_depth_core(locs, depth_core, z)
    assert depth_core == z
    ...
    assert zrange == top
    depth_core_new = DataInterpolationDriver.minimum_depth_core(locs, depth_core, z)
...
def test_find_top_padding(tmp_path):
    ...
    obj = DataInterpolationDriver.get_block_model(
        ws, "test2", locs, h, depth_core, pads, 1.1
    )

    top_padding = DataInterpolationDriver.find_top_padding(obj, h[2])
